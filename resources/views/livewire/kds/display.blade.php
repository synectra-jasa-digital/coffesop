<div class="h-full flex flex-col bg-gray-100" wire:poll.5s>
    <div class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 flex-shrink-0">
        <div class="flex items-center gap-4">
            <a href="{{ route('dashboard') }}" class="text-gray-500 hover:text-[#398263] transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" />
                </svg>
            </a>
            <div class="font-serif font-bold text-xl text-[#398263]">Kitchen Display</div>
        </div>
    ...
    <div class="flex-1 p-6 overflow-x-auto overflow-y-hidden">
        <div class="flex gap-6 h-full items-start">
            ...
            @forelse($activeOrders as $order)
                <!-- Ticket -->
                <div class="w-80 flex-shrink-0 flex flex-col max-h-full bg-white rounded-sm {{ $order->status === 'pending' ? 'border-t-4 border-t-red-500' : 'border-t-4 border-t-yellow-500' }} shadow-sm overflow-hidden">
                    ...
                    <div class="p-4 flex-1 overflow-y-auto space-y-4">
                        ...
                        <div class="flex justify-between items-start">
                            <div>
                                <div class="font-bold">{{ $item->quantity }}x {{ $item->product->name ?? 'Produk Dihapus' }}</div>
                                @if($item->notes)
                                <div class="text-sm text-gray-500 ml-4 border-l-2 border-gray-200 pl-2 mt-1">{{ $item->notes }}</div>
                                @endif
                            </div>
                        </div>
                        @endforeach
                    ...
                    <div class="p-4 border-t border-gray-100 bg-gray-50 flex gap-2">
                        ...
                        @else
                            <button wire:click="updateOrderStatus({{ $order->id }}, 'pending')" class="flex-1 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-bold py-3 rounded-sm transition-colors text-sm">
                                Batal
                            </button>
                            <button wire:click="updateOrderStatus({{ $order->id }}, 'completed')" class="flex-1 bg-[#398263] hover:bg-[#2C6B4F] text-white font-bold py-3 rounded-sm transition-colors">
                                Selesai
                            </button>
                        @endif
                    </div>
